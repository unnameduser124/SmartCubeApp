...
class PLLDBService(
    ...
    fun getPLLData(id: Long): PLLData? {
        ...
        with(cursor) {
            if (moveToFirst()) {
                val solveID =
                    getLong(getColumnIndexOrThrow(SolvesDatabaseConstants.PLLTable.SOLVE_ID_COLUMN))
                val duration =
                    getLong(getColumnIndexOrThrow(SolvesDatabaseConstants.PLLTable.DURATION_COLUMN))
                val moveCount =
                    getInt(getColumnIndexOrThrow(SolvesDatabaseConstants.PLLTable.MOVE_COUNT_COLUMN))
                val startStateID =
                    getLong(getColumnIndexOrThrow(SolvesDatabaseConstants.PLLTable.START_CUBE_STATE_ID_COLUMN))
                val endStateID =
                    getLong(getColumnIndexOrThrow(SolvesDatabaseConstants.PLLTable.END_CUBE_STATE_ID_COLUMN))
                val case =
                    getInt(getColumnIndexOrThrow(SolvesDatabaseConstants.PLLTable.CASE_COLUMN))
