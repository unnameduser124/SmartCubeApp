...
class F2LDBService(
    ...
    fun getF2LData(id: Long): F2LData? {
        ...
        with(cursor) {
            if (moveToFirst()) {
                val solveID =
                    getLong(getColumnIndexOrThrow(SolvesDatabaseConstants.F2LTable.SOLVE_ID_COLUMN))
                val duration =
                    getLong(getColumnIndexOrThrow(SolvesDatabaseConstants.F2LTable.DURATION_COLUMN))
                val moveCount =
                    getInt(getColumnIndexOrThrow(SolvesDatabaseConstants.F2LTable.MOVE_COUNT_COLUMN))
                val startStateID =
                    getLong(getColumnIndexOrThrow(SolvesDatabaseConstants.F2LTable.START_CUBE_STATE_ID_COLUMN))
                val endStateID =
                    getLong(getColumnIndexOrThrow(SolvesDatabaseConstants.F2LTable.END_CUBE_STATE_ID_COLUMN))
